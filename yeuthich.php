...
// --- PHẦN 1: XỬ LÝ AJAX (PHP nhận ID và trả về HTML sản phẩm) ---
if (isset($_POST['wishlist_ids'])) {
    ...
    // Làm sạch ID (chỉ lấy số) để tránh SQL Injection
    $ids = array_map('intval', $ids_raw);
    $ids_string = implode(',', $ids);

    // Truy vấn lấy sản phẩm theo danh sách ID
    $sql = "SELECT * FROM SAN_PHAM WHERE sanpham_id IN ($ids_string)";
...
?>
            <div class="product-card border p-4 rounded-lg shadow hover:shadow-lg transition relative group">
                ...
                <div class="mt-4">
                    <button type="button"
                        onclick="openModal(<?php echo $product['sanpham_id']; ?>, '<?php echo addslashes($product['ten']); ?>', <?php echo $product['gia']; ?>, '<?php echo $product['hinhAnh']; ?>')"
